...
ScriptCommander::ScriptCommander( const std::vector<std::string>& args )
{
	if( args.size() >= 2 && args[0] == "--commands" )
	{
		...
		if( !script.is_open() )
		{
			throw SCRIPT_ERROR( "Не удаётся открыть командный файл"s );
		}
		jso::json top;
		script >> top;
		...
		if( top.at( "enabled" ) )
		{
			...
			for( const auto& j : top.at("commands") )
			{
				...
				{
					throw SCRIPT_ERROR( "Неизвестная команда: "s + commandName );
				}
			}
			if( abort )
			{
				throw Completion( "Команды выполнены успешно" );
			}
		}
	}
}
...
ScriptCommander::Completion::Completion( const std::string& content ) noexcept
	:
	EngineException( 69,"@ScriptCommanderAbort" ),
	content( content )
{}
