...
namespace Dvtx
{
	...
	public:
		...
		static constexpr auto Bridge( VertexLayout::ElementType type,Args&&... args) noxnd
		{
			...
			assert( "Некорректный тип элемента" && false );
	...
	public:
		...
		const Element& Resolve() const noxnd
		{
			...
			assert( "Не удаётся верифицировать тип" && false );
	...
	private:
		// Необходимо для Bridge SetAttribute
	...
	private:
		// Включает установку пакета параметров нескольких параметров по индексу элемента
		...
		// Помощник для уменьшения дублирования кода в SetAttributeByIndex
		...
		void SetAttribute( char* pAttribute,SrcType&& val ) noxnd
		{
			...
			{
				assert( "Несоответствие типа атрибута параметра" && false );
			}
		}
	private:
		char* pData = nullptr;
	...
	public:
		...
		{
			assert( sizeof...(params) == layout.GetElementCount() && "Количество параметров не совпадает с количеством вершинных элементов" );
