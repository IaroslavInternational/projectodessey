...
DirectX::XMFLOAT3 ExtractEulerAngles( const DirectX::XMFLOAT4X4& mat )
{
	...
	euler.x = asinf( -mat._32 );                  // Тангаж
	if( cosf( euler.x ) > 0.0001 )                // Не на полюсах
	{
		euler.y = atan2f( mat._31,mat._33 );      // Рыскание
		euler.z = atan2f( mat._12,mat._22 );      // Крен
	}
	else
	{
		euler.y = 0.0f;                           // Рыскание
		euler.z = atan2f( -mat._21,mat._11 );     // Крен
	}
