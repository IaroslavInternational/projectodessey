...
void TexturePreprocessor::FlipYAllNormalMapsInObj( const std::string & objPath )
{
	...
	// Загрузка сцены из файла. obj, чтобы получить список карт нормалей в материалах
	Assimp::Importer imp;
	...
	// Просмотр материалов и обработка любых карт нормалей
...
{
	// Функция для обработки каждой нормали в текстуре
	...
	// Обработка каждого текселя в файле
...
{
	OutputDebugStringA( ("Валидация карты нормалей [" + pathIn + "]\n").c_str() );
	...
	const auto ProcessNormal = [thresholdMin,thresholdMax,&sum]( FXMVECTOR n,int x,int y ) -> XMVECTOR
	{
		...
		if( len < thresholdMin || len > thresholdMax )
		{
			...
			oss << "Не верная длина нормали: " << len << " at: (" << x << "," << y << ") нормаль: (" << vec.x << "," << vec.y << "," << vec.z << ")\n";
		...
		if( z < 0.0f )
		{
			...
			XMStoreFloat3( &vec,n );
			std::ostringstream oss;
			oss << "Не верно направлени Z нормали в: (" << x << "," << y << ") нормаль: (" << vec.x << "," << vec.y << "," << vec.z << ")\n";
			OutputDebugStringA( oss.str().c_str() );
		}
	...
	// Смещение выхода
	{
		...
		std::ostringstream oss;
		oss << "Смещение карты нормалей: (" << sumv.x << "," << sumv.y << ")\n";
		OutputDebugStringA( oss.str().c_str() );		
	}
}
